...
<section class="content">
  ...
  <div class="box">
    <div class="box-body">
      <div class="row">
       <div class="col-md-4">
        <form action="{{route('backend.category-edit',['id'=>$model->id])}}" method="POST" class="ng-pristine ng-valid">
          ...
          <div class="panel panel-default">
            <div class="panel-body">
              <div class="form-group">
                ...
                <input class="form-control" id="name" name="name" placeholder="Nhập tên danh mục ..." type="text" value="{{$model->name}}">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input id="slug" name="slug" class="form-control" placeholder="Nhập đờng dẫn tĩnh<..." type="text" value="{{$model->slug}}">
                 @if($errors->has('slug'))
            ...
            <div class="form-group">
                <label class="control-label">Danh mục cha</label>
                 <select name="parent" id="inputParent" class="form-control" required="required">
                   ...
                   @foreach ($cats as $cat)
                   @php
                     $selected = ($model && $model->parent == $cat->id) ? 'selected' : '';
                   @endphp
                ...
                <input type="hidden" name="image" id="image" />
                 <div class="thumbnail" id="thumbnail">
                  ...
                  <a href="#" class="thumb" data-field="image" data-view="thumb-view">
                   <img id="thumb-view" src="{{$model->image != null ?'uploads/'.$model->image: 'public/images/no-ig.png'}}" class="img-thumb">
                  </a>
                 </div>
            </div>
            <div class="form-group">
                <label class="control-label">Mô tả</label>
                <textarea name="descriptions" class="form-control" rows="3">{{$model->descriptions}}</textarea>
            </div>
            <div class="form-group">
                <label class="control-label">Trạng thái</label>
                 <select name="status" id="inputParent" class="form-control">
                 <option value="0" {{$model->status == 0 ? 'selected': ''}}>Không kích hoạt</option>
